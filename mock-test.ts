...
import { VncError } from './src/types';
...
async function runMockTests() {
  ...
  await test('Mock Server Creation with Valid Options', () => {
    ...
    if (!(server instanceof MockVncServer)) {
  ...
  await test('Mock Server Creation with Invalid Port', () => {
    ...
    try {
      new MockVncServer({ port: 0 });
    } catch (error) {
      if (error instanceof VncError && error.type === 'SYSTEM_ERROR') {
        errorThrown = true;
      }
    }
  ...
  await test('Mock Quality Settings Validation', () => {
    const server = new MockVncServer({ port: 5904 });
  ...
  await test('Mock Event Emitter Interface', () => {
    const server = new MockVncServer({ port: 5906 });
    ...
    // Test that eventFired variable is properly initialized
    if (typeof eventFired !== 'boolean') {
      throw new Error('eventFired variable not properly initialized');
    }
  })();

  // Test 6: Server lifecycle (start/stop)
  await test('Mock Server Lifecycle', async () => {
    const server = new MockVncServer({ port: 5907 });
  ...
  await test('Mock Client Simulation', () => {
    const server = new MockVncServer({ port: 5908 });
  ...
  await test('Mock Server Status', async () => {
    const server = new MockVncServer({ port: 5909 });
  ...
  await test('Mock Screen Info', () => {
    const server = new MockVncServer({ port: 5910 });
  ...
  await test('Mock Event Flow', async () => {
    return new Promise<void>((resolve, reject) => {
      const server = new MockVncServer({ port: 5911 });
