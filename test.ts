...
async function runTests() {
  ...
  await test('Server Creation with Valid Options', () => {
    ...
    if (!(server instanceof VncServer)) {
  ...
  await test('Quality Settings Validation', () => {
    const server = new VncServer({ port: 5904 });
  ...
  await test('Event Emitter Interface', () => {
    const server = new VncServer({ port: 5906 });
    ...
    // Test event firing - just check that eventFired is initialized
  ...
  await test('Server Status (Not Running)', () => {
    const server = new VncServer({ port: 5907 });
  ...
  await test('Screen Info Retrieval', () => {
    const server = new VncServer({ port: 5908 });
  ...
  await test('Server Lifecycle (Brief)', async () => {
    const server = new VncServer({ port: 5909 });
